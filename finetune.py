...
def main():
    ...
    finetune_dataset = dataset.get_dataset(args.dataset_name, args.n_views)

    finetune_loader = torch.utils.data.DataLoader(
        finetune_dataset, batch_size=args.batch_size, shuffle=True,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(finetune_loader), eta_min=0,
                                                           last_epoch=-1)

    #  It’s a no-op if the 'gpu_index' argument is a negative integer or None.
    with torch.cuda.device(args.gpu_index):
        simclr = SimCLR(model=model, optimizer=optimizer, scheduler=scheduler, args=args)
        simclr.finetune(finetune_loader)
